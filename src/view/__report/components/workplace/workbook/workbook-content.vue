...
import tabsForm from './tabs-form3.vue';
...
const fetchTaskData = () => {
	getMenuCourseData<ReportMenuCourse>(props.textBookTrees[1].id, 7).then((res) => {
		taskRecord.value = res;
	});
};

const fetchTabsData = () => {
	getMenuCourseData<ReportMenuLooseLeaf>(props.textBookTrees[2].id, 7).then((res) => {
