...
import { createResourceCalendarSchedule, exportCalendarSchedule, getResourceCalendarSchedule } from '@/axios';
...
onMounted(() => {
	getResourceCalendarSchedule(params.reportId, 1).then((res) => {
		data.value = res.data;
...
const handleSave = () => {
	...
	if (flag && data.value) {
		createResourceCalendarSchedule(data.value).then((res) => {
			data.value = res.data;
...
<template>
	<a-card size="small" style="height: calc(100vh - 200px)">
		...
		<template #extra>
			...
			<a-button type="primary" v-if="params.type === 'preview'" @click="exportCalendarSchedule(params.reportId, 1)"
				>导出</a-button
			>
		</template>
						...
						<template v-for="dayPartType in [1, 2, 3, 4, 5]">
							<tr>
								...
								<template v-else>
									...
									<td>
										<a-button type="link" :disabled="isDisable" danger @click="removeTimeSlot(item - 1, dayPartType, 0)"
											>删除</a-button
										>
									</td>
								</template>
