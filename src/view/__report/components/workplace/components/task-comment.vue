...
import { ReportAuditTableRecord, UpdateParams, ReportMenuCourse, AuditRecordData } from '@/view/__report';
import { message } from 'ant-design-vue';
import security from '@/utils/security.ts';
import useAxios, { getTableAudit, updateTableAudit, updateAuditRecord } from '@/axios';
...
const approveInfo = computed(() => {
	return props.taskForm?.tableFormData[0].auditRecordData;
});
...
const handleSave = () => {
	...
	params.tableFormData[0].editedFlag = true;
	axios.put('/report/course/updateMenuCourseData', params).then(() => {
...
onMounted(() => {
	tableId.value = props.taskForm?.tableFormData[0].tableId || '';
