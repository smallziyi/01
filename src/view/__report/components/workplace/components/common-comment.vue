...
import { ReportAuditTableRecord, UpdateParams, ReportMenuLooseLeaf, AuditRecordData } from '@/view/__report';
import { message } from 'ant-design-vue';
import security from '@/utils/security.ts';
import useAxios, { getTableAudit, updateAuditRecord, updateTableAudit } from '@/axios';
...
const tabsForm = inject('tabsForm') as ReportMenuLooseLeaf;
const activeKey = inject('activeKey') as Ref<number>;
const currentTableId = inject('currentTableId') as Ref<string>;
...
const approveInfo = computed(() => {
	return tabsForm?.tableFormData[activeKey.value - 1].auditRecordData;
});
...
const handleSave = () => {
	// 是评价，评价本身标记，否则标记下一个页签
	if (activeKey.value === 6) {
		tabsForm.tableFormData[5].editedFlag = true;
	} else {
		if (!tabsForm.tableFormData[activeKey.value].editedFlag) {
			tabsForm.tableFormData[activeKey.value - 1].editedFlag = true;
		}
	}
	const currentData = { ...tabsForm };
	axios
		.put('/report/course/updateMenuCourseData', {
			...currentData,
			tableFormData: [currentData.tableFormData[activeKey.value - 1]],
		})
		.then(() => {
...
const handleApprove = () => {
	...
	updateAuditRecord(params).then(() => {
		message.success('已审阅！');
		emits('approved', record);
	});
};
...
onMounted(() => {
	getTableAudit({ tableId: currentTableId.value }).then((res) => {
		auditRecord.value = res;
	...
	<div class="text-right">
		<a-space class="py-2">
			...
			<div>
				...
				<a-button
					v-if="$props.type === 'audit' && !approveInfo"
					class="mr-5"
					size="small"
					type="primary"
					@click="handleApprove"
