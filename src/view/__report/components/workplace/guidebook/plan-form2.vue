...
onMounted(() => {
	...
	data.value.tableFormData[1].data.stepDescriptionList.forEach((item) => {
...
const tableId = data.value.tableFormData[1].tableId;
const form = computed(() => {
	return data.value.tableFormData[1].data;
});
const learningPoint = computed(() => {
	return data.value.tableFormLearnPointsData.find((item) => item.tableId === tableId)?.sentencePartList!;
});

const source = [form.value.studyName, form.value.contextName, form.value.taskName, data.value.name, '计划单'].join(
	...
	if (result.some((item) => item.isMark)) {
		if (questions.value.every((item) => item.learnPointsCellId !== id)) {
			questions.value.push({
				...
				source: source,
				title: `在具体步骤"${name}"中：${result.map((var1) => (var1.isMark ? '____' : var1.text)).join('')}`,
				analysis: '',
				sort: 1,
				options: [
					{
			...
			if (!question) return;
			question.title = `在具体步骤"${name}"中：${result.map((var1) => (var1.isMark ? '____' : var1.text)).join('')}`;
...
const handleSaveContent = async () => {
	try {
		...
		await axios.put('/report/course/updateMenuCourseData', {
			...data.value,
			tableFormData: [data.value.tableFormData[1]],
...
<template>
	<table class="w-full table-fixed">
		<tbody>
			<tr>
				<td colspan="12" class="table-title">{{ data.name }}的计划单</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" class="table-title" :rowspan="form.planMethodList.length">计划制定的方式</td>
				<td colspan="10" class="table-title text-left" v-if="form.planMethodList.length">
					{{ form.planMethodList[0] }}
				</td>
			</tr>
			<template v-for="(item, index) in form.planMethodList">
				<tr v-if="index !== 0">
			...
			<tr>
				<td rowspan="2" colspan="2" class="table-title">计划单的评分</td>
