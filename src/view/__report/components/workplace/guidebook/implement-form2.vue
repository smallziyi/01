...
onMounted(() => {
	...
	data.value.tableFormData[3].data.stepDescriptionList.forEach((item) => {
...
const tableId = data.value.tableFormData[3].tableId;
const form = computed(() => {
	return data.value.tableFormData[3].data;
});
const learningPoint = computed(() => {
	return data.value.tableFormLearnPointsData.find((item) => item.tableId === tableId)?.sentencePartList!;
});

const _source = [form.value.studyName, form.value.contextName, form.value.taskName, data.value.name, '实施单'].join(
	...
	if (result.some((item) => item.isMark)) {
		if (questions.value.every((item) => item.learnPointsCellId !== id)) {
			questions.value.push({
				...
				source: _source,
				title: `在实施的具体步骤"${name}"中：${result.map((var1) => (var1.isMark ? '____' : var1.text)).join('')}`,
				analysis: '',
				sort: 1,
				options: [
					{
			...
			if (!question) return;
			question.title = `在实施的具体步骤"${name}"中：${result.map((var1) => (var1.isMark ? '____' : var1.text)).join('')}`;
...
const handleSaveContent = async () => {
	try {
		...
		data.value.tableFormData[3].editedFlag = true;
		await axios.put('/report/course/updateMenuCourseData', {
			...data.value,
			tableFormData: [data.value.tableFormData[3]],
			tableFormLearnPointsData: [{ sentencePartList: learningPoint.value, tableId: tableId }],
		});
		data.value.tableFormData[3].auditRecordData = null;
