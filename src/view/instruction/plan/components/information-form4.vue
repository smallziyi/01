...
onMounted(() => {
	getTableFormQuestion({ tableId: props.data.tableFormData[0].tableId, studyFlag: false }).then((res) => {
...
<template>
	<template v-if="$props.data.tableFormData[0].showFlag">
		...
		<course-test :table-form-type="1" :table-id="tableId" :questions="questions"></course-test>
	</template>
	<m-empty v-else />
</template>
