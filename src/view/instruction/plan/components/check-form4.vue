...
const tableId = props.data.tableFormData[4].tableId;
const form = computed(() => {
	return props.data.tableFormData[4].data;
});
...
onMounted(() => {
	getTableFormQuestion({ tableId: props.data.tableFormData[4].tableId, studyFlag: false }).then((res) => {
...
<template>
	<template v-if="$props.data.tableFormData[4].showFlag">
		...
		<course-test :table-form-type="5" :table-id="tableId" :questions="questions"></course-test>
	</template>
	<m-empty v-else />
</template>
