...
const tableId = props.data.tableFormData[2].tableId;
...
onMounted(() => {
	getTableFormQuestion({ tableId: props.data.tableFormData[2].tableId, studyFlag: false }).then((res) => {
...
<template>
	<template v-if="$props.data.tableFormData[2].showFlag">
		...
		<course-test :table-form-type="3" :table-id="tableId" :questions="questions"></course-test>
	</template>
	<m-empty v-else />
</template>
