...
import useAxios, { getReportInfo, reportUsableDropdown, getGroupTree } from '@/axios';
...
const p = ref<Partial<TeachPlan>>({ clientId: user.value!.clientId, status: 4 });
const t = ref<
	Partial<{
		a: string;
		b: string;
...
const handleChange = async (value: SelectValue, _option: DefaultOptionType | DefaultOptionType[]) => {
	...
	if (value) {
		...
		axios
			.get('/report/course/getMenuCourseAuditTree', { params: { reportId: value, detailsFlag: false } })
			.then((res) => {
...
const handleChangeTeachDate = async () => {
	if (p.value.teachDate) {
		axios
			.get<string[]>('/teach/plan/getTimeAndTeacherAlreadyTimeSlot', {
				params: {
					teacherId: p.value.teacherId,
					teachDate: p.value.teachDate.toDate().toLocaleDateString().replace('/', '-').replace('/', '-'),
				},
			})
			.then((res) => {
				disableTime.value = res.data;
			});
	}
...
const handleSelectTime = (value: ValueType, _selectOptions: DefaultOptionType[] | DefaultOptionType[][]) => {
	if (value && p.value.teachDate) {
		...
		axios
			.get<{ classroomIds: string[]; groupIds: string[] }>('/teach/plan/getTimeAndTimeSlotAlreadyGroupAndClassroom', {
				params: {
					teachDate: new Date().toLocaleDateString().replace('/', '-').replace('/', '-'),
					timeSlotId: value[1] as string,
				},
			})
			.then((res) => {
				const { classroomIds, groupIds } = res.data;
...
const handleCreate = () => {
	form
		.value!.validate()
		.then(() => {
			axios.post('/teach/plan', p.value).then(() => {
...
onMounted(() => {
	...
	axios.get<Classroom[]>('/resource/classroom', { params: { disable: false } }).then((res) => {
...
<template>
	<a-drawer v-model:open="open" width="50%" :close-icon="false" :destroy-on-close="true" :mask-closable="false">
		<template #title>
		...
		<a-form :model="p" ref="form">
			...
			<a-form-item label="课程图片" required>
				<template v-if="t.e">
					<a-image :src="t.e" class="w-[109.5px] h-[146px]" />
				</template>
			</a-form-item>
								...
								?.dayPartDataList.map((dayPart) => {
									return {
										...
										children: dayPart.timeSlotData
											...
											.map((slot) => {
												return {
													label: slot.timeSlotName + ': ' + slot.startTime + '~' + slot.endTime,
													value: slot.id,
						...
						:options="
							classrooms.map((item) => {
								return {
									...
									disabled: disableRoom.includes(item.id),
								};
							})
						"
		...
		<template #footer>
			<a-space class="float-end">
				...
				<a-button type="primary" @click="handleCreate">确定</a-button>
			</a-space>
		</template>
	</a-drawer>
</template>
