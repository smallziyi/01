...
const tableId = props.data.tableFormData[1].tableId;
const form = computed(() => {
	return props.data.tableFormData[1].data;
});
...
onMounted(() => {
	getTableFormQuestion({ tableId: props.data.tableFormData[1].tableId, studyFlag: false }).then((res) => {
...
<template>
	<template v-if="$props.data.tableFormData[1].showFlag">
		...
		<course-test :table-form-type="2" :table-id="tableId" :questions="questions"></course-test>
	</template>
	<m-empty v-else />
</template>
