...
import { getAnswerRecord, getStepQuestion, studentAnswerQuestion } from '@/axios';
...
const questions = ref<ReportCourseQuestion[]>([]);
	...
	getAnswerRecord({
		...
		if (res.answerDataList) {
			questionsRecord.value = {
				...
				answerDataList: sortArrayByOrder(res.answerDataList, questions.value),
			};
...
const buttonClass = reactive({
	...
	done: 'border-gray-500 bg-gray-100 text-gray-500 :hober:bg-gray-300',
...
onMounted(() => {
	getStepQuestion({ menuCourseId: props.menuId, studyFlag: true }).then((res) => {
		questions.value = res.data;
		answer.value = res.data.map((item: any) => {
			return {
				questionId: item.id || '',
				answerId: '',
				optionIds: item.options.map((i: any) => i.id) || [''],
			};
		});
		handleCheck();
	});
...
<template>
	<div class="overflow-y-auto" style="height: calc(100vh - 340px)" :style="{ background: cardBgColor }">
		<div v-if="!submitFlag" class="h-full grid grid-cols-3 gap-2 p-4">
			<a-card v-if="questions.length > 0">
				<div class="border-[1px] h-[68px] mb-7 border-blue-500 rounded-md flex justify-center items-center">
					<sand-svg class="mr-4"></sand-svg>
					<div class="text-blue-500 text-lg">请认真作答</div>
				</div>
		...
		<div v-else class="h-full grid grid-cols-3 gap-2 p-4">
			<a-card v-if="questions.length > 0">
			...
			<div class="col-span-2 relative">
				<a-card class="h-full">
					...
					<div>
						<a-button class="my-4 flex items-center" shape="round" @click="showAnalysis = !showAnalysis">
							解析
