...
<template>
    <div class=" w-[320px] h-[360px]  ">
        <dv-border-box12 class="w-full h-full">
            <div class="justify-content align-items w-full flex" v-if="title !== ''">
                <Decoration1 style=" width: 30px; height: 30px;" />
                <div class="item_title">
                    <div class="zuo"></div>
                    <dv-decoration-9 style="width:30px;height:30px; "></dv-decoration-9>
                    <span class="title-inner"> {{ title }} </span>
                    <div class="you"></div>
                </div>
                <Decoration1 style=" width: 30px; height: 40px;" />
            </div>
            <div :class="title !== '' ? 'item_title_content' : 'item_title_content_def'">
                <div v-if="typid === -1" class=" w-[100%] h-[100%] ">
                    <div class="w-full pl-2 pr-2">
                        <div class="w-full h-[120px] mt-3 indent-7">
                            <div class="scroll-container pr-4 pl-1" id="contentDiv" style=" overflow: hidden; overflow-y: auto; height: 290px;"></div>
                        </div>
                    </div>
                </div>

                <div v-else-if="typid === -2">
                    <div class="w-[100%] h-[120px] pl-2 pr-2 pt-7">
                        <a-carousel arrows autoplay>
                            <template #prevArrow>
                                <div class="custom-slick-arrow"
                                    style="top:170px;left: 10px; background-color: transparent;">
                                    <left-circle-outlined />
                                </div>
                            </template>
                            <template #nextArrow>
                                <div class="custom-slick-arrow"
                                    style="top: 170px; right:10px;background-color: transparent;">
                                    <right-circle-outlined />
                                </div>
                            </template>
                            <div class="w-full h-[270px]" v-for="item in images">
                                <h3><img class="w-[100%] h-[265px]" :src="item.fileUrl" alt="" srcset=""></h3>
                            </div>
                        </a-carousel>
                    </div>
                </div>
