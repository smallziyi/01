...
import { updateClassroom, getClassroomDetails } from '@/axios';
import {
	ClassroomAdd,
	sourceOption,
	natureOption,
...
watch(
	...
	(newVal) => {
		if ([1, 2, 3].includes(newVal as number) && !classroomData.value.source) {
			classroomData.value.source = 1;
		} else if ([4, 5, 6].includes(newVal as number) && !classroomData.value.source) {
			classroomData.value.source = 2;
		}
	},
);
const sourceChange = (value: SelectValue, _option: DefaultOptionType | DefaultOptionType[]) => {
	if (value) {
...
onMounted(() => {
	mangerIdOption.value = props.mangerOption;
	getClassroomDetails(props.classroomId).then((res) => {
		classroomData.value = res;
		fileList.value = [
			{
				uid: classroomData.value.coverId || '',
				name: classroomData.value.coverUrl || '',
				status: 'done',
				url: classroomData.value.coverUrl || '',
			},
		];
	});
});
	...
	<a-drawer :closable="false" v-model:open="open" destroy-on-close width="864" :maskClosable="false">
		...
		<a-form ref="formRef" :model="classroomData" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
			<div class="grid grid-cols-2 gap-x-4">
				...
				>
					<a-select
						v-model:value="classroomData.source"
