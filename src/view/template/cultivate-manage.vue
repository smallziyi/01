...
const type = ref<number>(REPORT_TYPE.CULTIVATE);
...
<template>
	...
	<a-card :body-style="{ padding: '16px' }" :bordered="false" style="height: calc(100vh - 206px)">
		<template #title>
			<div class="card-title">人才培养方案模板列表</div>
		</template>
		<template #extra>
			<div>
			...
			<template #bodyCell="{ column, record }: TableRow<TemplateFile>">
				...
				<template v-if="column.key === 'operation'">
					<a-space>
						...
						<a v-if="!record.deleteFlag" class="px-0" type="link" @click="onUnBind(record.id)">解绑菜单</a>
