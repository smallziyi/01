...
const type = ref<number>(REPORT_TYPE.SURVEY);
...
<template>
	...
	<a-card :body-style="{ padding: '16px' }" :bordered="false" style="height: calc(100vh - 206px)">
		<template #title>
			<div class="card-title">企业调研模板列表</div>
		</template>
		<template #extra>
			<div>
		...
		>
			...
			<template #bodyCell="{ column, record, index }: TableRow<TemplateFile>">
				...
				<template v-if="column.key === 'url'">
					<a-button type="link" @click="$download(record.fileId)">{{ record.fileName }}</a-button>
				</template>
				<template v-if="column.key === 'operation'">
					<a-space>
						...
						<a v-if="!record.deleteFlag" type="link" @click="onUnBind(record.id)"> 解绑菜单</a>
