...
<template>
	...
	<a-card :body-style="{ padding: '16px' }" :bordered="false" style="min-height: calc(100vh - 206px)">
		<template #title>
			<div class="card-title">领导列表</div>
		</template>
		<template #extra>
			<user-add-batch user-type="leader" @success="reset"></user-add-batch>
		...
		>
			...
			<template #bodyCell="{ column, record, index }: TableRow<User>">
				<template v-if="column.key === 'index'">
					<span class="font-mono">{{ (pagination.current! - 1) * pagination.pageSize! + index + 1 }}</span>
				</template>
	...
	<user-add v-if="state.createOpen" role-code="leader" v-model:open="state.createOpen" @success="reset" />
</template>
